...
class BooksApp extends React.Component {
    ...
    moveToShelf = (bookToMove, toShelfId) => {
        let fromShelfId = bookToMove.shelfId;// old shelfId
        if (!this.state[fromShelfId] && !this.state[toShelfId]) {
        ...
        this.setState(prevState => {
                let newState = {};
                // remove from old...
                newState[fromShelfId] = prevState[fromShelfId].filter(book => book.id !== bookToMove.id);
                // ... and add to new shelf
                bookToMove.shelfId = toShelfId;
                prevState[toShelfId].push(bookToMove);
                newState[toShelfId] = prevState[toShelfId];
                return newState;
            }
        );
    };
